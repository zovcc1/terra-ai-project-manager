...
> Auto-maintained by OpenWolf. Last scanned: 2026-06-24T11:59:29.217Z
...
- `OpenAiClient.java` — Component: OpenAiClient (~775 tok)
...
- `TaskController.java` — RestController: TaskController (9 endpoints) (~1323 tok)
...
- `AiSettingsRequest.java` — Class: AiSettingsRequest (~71 tok)
...
- `AiSettingsResponse.java` — Class: AiSettingsResponse (~82 tok)
...
- `AiSettings.java` — Entity: AiSettings (~156 tok)
...
- `GlobalExceptionHandler.java` — RestController: GlobalExceptionHandler (~1070 tok)
...
- `AiSettingsService.java` — Returns the decrypted API key for use by the LLM client. (~1240 tok)
...
- `application.properties` (~439 tok)
...
- `app-shell.tsx` — navByPersona (~4197 tok)
...
- `api.ts` — Exports getAuthToken, setAuthToken, removeAuthToken, apiFetch + 59 more (~4269 tok)
...
- `websocket.ts` — WebSocket client — SockJS + STOMP (~2118 tok)
...
- `__root.tsx` — NotFoundComponent — uses useRouter (~1315 tok)
...
- `ai-settings.tsx` — Route — renders form — uses useState, useQuery, useEffect, useMutation (~3675 tok)
...
- `dashboard.tsx` — Route — uses useQuery (~1904 tok)
- `kanban.tsx` — Route — renders modal (~5410 tok)
- `projects.$projectId.tsx` — Route — renders modal — uses useState, useQuery, useMutation (~7938 tok)
- `tasks.$taskId.tsx` — Route (~5204 tok)
...
- `kanban.tsx` — Route — renders modal (~4552 tok)
- `my-tasks.tsx` — Route — uses useQuery (~1282 tok)
- `projects.tsx` — Route — renders modal — uses useState, useQuery, useMutation (~7646 tok)
- `tasks.$taskId.tsx` — Route (~5204 tok)
